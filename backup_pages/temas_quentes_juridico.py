#!/usr/bin/env python3
"""
Página Temas Quentes - Monitoramento Jurídico Específico (Versão Corrigida)
...
def render_post_card_native(post, platform="instagram"):
    """Renderiza card de post usando componentes nativos do Streamlit"""
    ...
    with st.container():
        ...
        # Título e autor
        st.markdown(f"### {post.get('title', 'Título não disponível')}")
        st.markdown(f"👤 **{post.get('author', 'Autor desconhecido')}**")
        ...
        if st.button(f"▶️ Ver Original", key=f"btn_{post.get('id', 'unknown')}"):
            st.markdown(f"[Abrir Link]({post.get('link', '#')})")

def render_section_header_native(title, subtitle, icon):
    """Renderiza cabeçalho de seção usando componentes nativos"""
    st.markdown(f"""
    <div style="
        ...
        color: white;
    ">
        <h2 style="margin: 0; font-size: 24px; color: white;">{icon} {title}</h2>
        <p style="margin: 5px 0 0 0; opacity: 0.9; color: white;">{subtitle}</p>
    </div>
    """, unsafe_allow_html=True)

def render_temas_quentes_juridico():
    ...
    render_section_header_native(
        "Dr. Willian Godoy", 
        "Últimas 5 postagens do Instagram", 
        "👨‍⚖️"
    )
    ...
    render_section_header_native(
        "TikTok for Business", 
        "Top 5 anúncios jurídicos - Brasil", 
        "🎯"
    )
